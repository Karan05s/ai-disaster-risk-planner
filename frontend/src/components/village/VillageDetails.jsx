...
const VillageDetails = ({
  ...
  const [activeTab, setActiveTab] = useState("AI_RISK"); // "AI_RISK" | "WEATHER"
  ...
  }, [village?.id, village?.lat, village?.lng, village?.openWeather, village?.isCustomLocation]);
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: "12px", marginTop: "6px", color: "#64748b", lineHeight: "1.4" }}>
          Click any village pin, shelter, or any custom point on the map to inspect real-time weather, 12-hour forecasts, and AI disaster risk diagnostics.
        </div>
      </div>
    );
  }

  // Handle Custom Pin / Relocation Site selection
  ...
  const suitableSites = relocationSites
    .filter(
      (site) =>
        ...
        site.availableCapacity >= (village.population || 500)
    )
    .map((site) => ({
      ...site,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span>📊</span>
          <span>{isShelter ? "Shelter Info" : "AI Risk & XAI"}</span>
        </button>
        ...
        >
          <span>⛅</span>
          <span>Live Weather & 12h</span>
        </button>
      </div>

      {/* TAB 1: AI RISK & EVACUATION CONTENT */}
      {activeTab === "AI_RISK" && (
        <>
          {/* QUICK WEATHER CALLOUT BANNER */}
          <div
            onClick={() => setActiveTab("WEATHER")}
            style={{
              marginBottom: "12px",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <span>⛅</span>
              <span><strong>Live Atmospheric Telemetry:</strong> Tap for 12h forecast</span>
            </div>
            <span style={{ fontWeight: "700", color: "#0284c7" }}>View →</span>
          </div>
            ...
            >
              <div>
                ...
                <strong style={{ color: riskColor, fontSize: "17px" }}>
                  {village.riskLevel || "MEDIUM"} {village.riskScore ? `(${village.riskScore.toFixed(1)}/100)` : ""}
                </strong>
              </div>
          ...
          {!isShelter && !isCustomPin && (
            ...
            >
              ...
              <p style={{ margin: 0, fontSize: "11.5px", color: "#3b0764", lineHeight: "1.45" }}>
                {village.aiSummary || (
                  `${village.name} in ${village.district || "the district"} (${village.state || "India"}) exhibits acute vulnerability driven by ${village.dominantFactor || "Hazard Exposure"}, placing ${(village.population || 4500).toLocaleString()} residents in immediate risk. Hungarian evacuation protocol recommends staged relocation to ${recommendedSite?.name || "the nearest safe relief facility"}.`
                )}
              </p>
            </div>
          )}
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                ...
                <span style={{ fontSize: "10.5px", color: "#15803d", background: "#dcfce7", padding: "1px 6px", borderRadius: "10px", fontWeight: "600" }}>
                  State: {recommendedSite?.state || village.state || "India"}
                </span>
              </div>
          ...
          {/* SHELTER DIRECT STATS (If shelter clicked) */}
          {isShelter && (
            <div
              style={{
                padding: "12px",
            ...
            >
              <div style={{ fontSize: "12px", color: "#166534", marginBottom: "8px" }}>
                <strong>Shelter Operational Status:</strong> {village.status || "AVAILABLE"}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px", fontSize: "11.5px", color: "#15803d" }}>
                <div>Total Capacity: <strong>{village.capacity?.toLocaleString()}</strong></div>
                <div>Available: <strong>{village.availableCapacity?.toLocaleString()}</strong></div>
              </div>
            </div>
          )}
      ...
      {/* TAB 2: LIVE WEATHER & 12-HOUR HOURLY FORECAST */}
      {activeTab === "WEATHER" && (
        <WeatherReport
          lat={village.lat}
          lng={village.lng}
